...
Write a function named getNames that, given an array of people objects, uses map to return an 
array of names reversed.
...
const getNames = (arr) => {
  ...
  arr.map(val=>{
    ...
    newArr.push(val.name.split('').reverse().join(''));
  });
  ...
  // Solution code here...
      ...
      let sec = input.forEach((val,idx)=>{
          ...
          count++;
                ...
                val.splice(count,1,0);
                  count--;
            } else{
              
            val.splice(idx2,1,Math.pow(2, val2));
